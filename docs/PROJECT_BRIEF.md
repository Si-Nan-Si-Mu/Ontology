...
| [PRESENTATION_SHOWCASE.md](PRESENTATION_SHOWCASE.md)                     | **展演文档**：内容、目的、演示脚本、未来方向；**§3.4** 信用/心理科技/社工防御的跨领域效用与边界；伦理与 Q&A                |
| [PITCH_ONE_PAGER.md](PITCH_ONE_PAGER.md)                               | 答辩一页纸（约 1 分钟口播结构）                            |
| [TECH_SPEC.md](TECH_SPEC.md)                                           | API、幂等、Neo4j 约束、**§7 安全与合规**、第一层 §5.9、验收清单                         |
...
### 2.1 跨领域效用：信用体系、心理干预、社会工程

本节说明 POG 路线在**相邻社会技术议题**上的**可能贡献**与**必须遵守的边界**（详细口播与 Q&A 见 [`PRESENTATION_SHOWCASE.md`](PRESENTATION_SHOWCASE.md) §3.4）。

| 议题 | 创新取向 | 实际效用（合规场景） | 红线 |
| --- | --- | --- | --- |
| **信用与声誉治理** | 用图+本体承载「材料—批次—假设—证据」，替代或补充**不可审计的黑盒画像** | 内部合规、研究复现、客服/争议处理中的**可解释材料层**；降低隐性标签传播 | **不得**作为征信、核保、招聘等高风险决策的**唯一依据**；须人工复核与依法告知 |
| **心理干预与心理科技** | 观测层与推断层分离，假设节点可版本化、可撤回 | 教练/EAP 工作台：**个案概念化、多源材料对齐、带引用的反思摘要**（未来图约束 LLM） | **非**诊断与治疗；来访者数据须同意与执业规范；危机须转介 |
| **社会工程（安全向）** | 将话术与关系压力点**结构化**，支撑红队剧本与复盘 | **社工防御演练**、安全意识课程、高压沟通仿真数据集 | **禁止**攻击性社工、欺诈辅助；对外统一「防御/教育/仿真」表述（见 §5.3） |
