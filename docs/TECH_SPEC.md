...
本文与 [`backend/ONTOLOGY.yaml`](../backend/ONTOLOGY.yaml) 同步；实现以 YAML 为**语义真源**，Neo4j 为**运行库**。
...
### 7.2 合规与跨领域使用边界（产品 / 法务对齐）

实现与对外 API 设计须与 [`ETHICS_AND_DATA.md`](ETHICS_AND_DATA.md) 一致；路演级叙事见 [`PRESENTATION_SHOWCASE.md`](PRESENTATION_SHOWCASE.md) **§3.4**，简报级表格见 [`PROJECT_BRIEF.md`](PROJECT_BRIEF.md) **§2.1**。技术侧应落实的约束包括：

| 领域 | 技术含义（摘要） | 规格要求 |
| --- | --- | --- |
| **信用与声誉 / 高风险决策** | 图谱与导出包可作为**可审计附件**，不等同于法定征信或自动化准入分数 | 不在 API 默认响应中输出「信用分」「录用建议」等高风险标签；若客户二次开发用于招聘/信贷/核保，须在集成文档与 ToS 中重申**禁止作为唯一依据**（见伦理 §2.2）。 |
| **心理干预与心理科技** | 辅助材料结构化、观测层与推断层分离 | 导出与 UI 区分**事实字段**与**假设/推断**；`POST /narrate`（§5.8–5.9）须支持**证据引用与缺图显式声明**，避免生成诊断式措辞（见伦理 §2.4）。 |
| **社会工程（安全向）** | 仅服务于**防御演练、安全意识教育、沟通仿真** | 功能与文档用语与「红队授权、范围限定、审计日志」绑定；不提供面向未授权目标的画像或攻击辅助（见伦理 §2.3）。 |

**第一层（§5.9）补充**：`GraphContextPack` 与叙述类接口的裁剪日志须可关联 `subject_id` / `batch_id`，便于争议核查；完整子图正文不进公共日志（与 §5.9 验收及 §7.1 一致）。
...
*版本：0.2 — 随 `backend/ONTOLOGY.yaml` 演进。*