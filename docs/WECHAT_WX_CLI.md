...
- `chat`：会话名  
- `is_group`：是否群聊  
- `username`：私聊时为**对端**微信 wxid（如 `wxid_bhua3dlia7tu22`），POG 以此作为 Person 主键，避免不同会话因昵称相同被合并  
- `messages`：数组，每项含 `time`、`sender`、`content`（字符串）；常见为 `sender` 为空表示本机发言；若会话名不在 sender 列表且仅有「空 + 一个昵称」，则**该昵称**为本机、空 sender 为对端（见 `infer_local_sender_ui_label`）  
- 本机 wxid 须在 `backend/.env` 配置 `WX_LOCAL_USERNAME`（可从 `%USERPROFILE%\.wx-cli\config.json` 的 `db_dir` 路径中 `wxid_…` 文件夹名取得）
...
```bash
wx export "慕柔小姐" -n 20000  --format json -o chat_lmr.json
```
...
4. 设置 **正式导出条数**（入库时 `wx export -n`，默认 2000；与预分析样本无关）。已选本机会话时，底部「导入聊天 JSON」**始终按此处条数重新 export**，不会使用已上传 JSON 文件内的旧条数。超长会话请同步增大 `WX_CHAT_IMPORT_MAX_MESSAGES` 与 `WX_CLI_TIMEOUT_SEC`。
...
| 群聊预分析显示 `wxp_…` 而非 `wxid_…` | 群成员主键来自 `wx members`（需 `WX_CLI_ENABLED=true` 且会话名与 `wx export` 一致）及消息正文挖掘；未解析时会退回临时哈希。重新预分析或开启本机 wx-cli。 |
| 导入条数与微信「聊天信息」总条数不一致                   | **统计口径不同**：① `wx export -n` 与配置上限只是上界，实际 ≤ 会话存量；② 规范化会丢弃**无正文**消息（响应里 `wx_cli.messages_raw_in_export` / `messages_normalized_in_export` / `messages_dropped_no_body`）；③ 人格摘要只用部分消息（`messages_used_for_digest`）。单份 JSON 原始消息上限由 `**WX_CHAT_IMPORT_MAX_MESSAGES`**（默认 10 万）控制，后端规范化按块合并后仍**一次性**全量送入摘要。 |
