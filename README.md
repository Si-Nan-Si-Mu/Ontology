...
**完整步骤（虚拟环境、Neo4j 校验、清空库、前后端命令）见：[`docs/OPERATIONS.md`](docs/OPERATIONS.md)。**

极简摘要：

1. **Neo4j**：准备实例；在 `backend/.env` 中配置 `NEO4J_URI`、`NEO4J_USER` / `NEO4J_USERNAME`、`NEO4J_PASSWORD`、`NEO4J_DATABASE`（见 `.env.example`）。  
2. **后端**：`cd backend` → 安装 `requirements.txt` → `uvicorn app.main:app --host 127.0.0.1 --port 8000 --reload`。  
...
- **结果导出**：`GET /api/v1/person/{subject_id}/persona-export?format=json|text`。  
- **图节点分页浏览**：`GET /api/v1/graph/nodes`（前端「图」页）。  
- **清空开发库**：`python backend/scripts/neo4j_wipe.py --dry-run` / `--yes`（在 `backend` 目录下执行，详见运维文档）。
